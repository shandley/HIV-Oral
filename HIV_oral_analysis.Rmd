...
p.prevdf1 <- ggplot(prevdf1, aes(TotalAbundance, Prevalence / nsamples(ps1),color=Class)) +
...
dist.hell <- dist(dat.hell, method = "euclidean", diag = TRUE, upper = TRUE)
pair.hell <- data.frame(t(combn(rownames(as.matrix(dist.hell)),2)), as.numeric(dist.hell))
head(pair.hell)

dat.pa <- decostand(otu.table, method = "pa")
dist.pa <- dist(dat.pa, method = "euclidean", diag = TRUE, upper = TRUE)
pair.pa <- data.frame(t(combn(rownames(as.matrix(dist.pa)),2)), as.numeric(dist.pa))
head(pair.pa)
