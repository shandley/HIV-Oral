...
knitr::opts_chunk$set(fig.width=6, fig.height=6, fig.path='figures/HIV_oral_16S_', dev='pdf',
...
setwd("~/Dropbox/research/AIDS/HIV/presti/HIV_oral/dada2/")

```

```{r, Load Libraries, include=FALSE}
# Pacakges needed for analysis
library("ggplot2")
...
library("biobroom")
packageVersion("biobroom")

# Packages being used for testing
library("caret")
...
#########################################
#                                       #
# Factor reclassification  & reordering #
#                                       #
#########################################

It is useful to dedicate a section of the analysis to factor editing, reclassification and reordering. This is useful to remove characters that may be part of the mapping file, but unwanted in plots (such as _) and also to refactor levels so they appear in a desired order in plot facets or differential abundance analysis.
...
This is a series of steps to detect and remove samples that are not useful for analysis. For example, if there are any samples that have 0 or a "few" reads, they can normally be removed. In addition, you can remove specific taxa, for example, I frequently remove samples classified as mitochondira and chloroplast.

A series of plots are also generated to provide a view of the per sample sequencing depth. These can be useful for determining a cut-off to remove samples with low sequence number.

```{r Summary Statistics, fig.height=4, fig.width=8}
# Preprocessing
...
This is useful for assessing and removing taxa that are infrequently present in samples. It can be useful to remove these samples for statistical tests (e.g. DeSeq2). This may be less useful for assessing alpha and beta diversity, so it is useful to keep one file of all of the data, and one with the low-prevlant samples removed and compare results between the two.
...
Commonly used data transformations. May or may not be useful for downstream analysis, but are useful to prepare ahead of time and give a could indication of the normality of the data and how various transformations effect the data.

```{r Data transformations, fig.width=8, fig.height=6}
# You will need to repeat these for the subsets if you want to analyze transformed subsets
...
# Visual inspection of log transformation
...
# Note: log transformation reasonably shifts the data to normality and should be used for downstream analysis when appropriate
...
Subsetting restructures the data into smaller components for independent analysis. Of note, many plots can be created in ggplot2 using facet_grid or facet_wrap which would alleviate the need to generate these subsets. But other analysis, such as DeSeq2 differential abundance testing benefit from having well-defined data subsets.

Of note, we are subsetting the original data prior to transformation. If you wish to analyze transformed data, you will need to apply appropriate functions to each subset.
...
Barplots displaying bacterial phyla present in samples. It is useful to filter out very low abundant taxa which are minor components. They contribute very little to the visualization and clutter the bars and legends.

This code will also rename the ggplot2 taxaonomic values (e.g. p__Actinobacteria) to the more simple 'name only' (e.g. Actinobacteria).
...
#############################
#                           #
# Sample Outlier Evaluation #
#                           #
#############################

An MDS plot of Bray-Curtis dissimilarity can be useful to assess if there are obvious sample outliers in the data. If outliers are suspected based on this plot one can consider their removal. Removal of outliers should be judge carefully.

```{r Sample Outlier Evaluation, fig.width=8, fig.height=6}
# Outlier evaluation
out.bray.log <- ordinate(ps0.log, method = "MDS", distance = "bray")
p.MDS.outlier <- plot_ordination(ps0.log, out.bray.log, color = "Week", shape ="VL_category") +
  theme_bw() +
  geom_point(size = 5) +
  ggtitle("MDS of Bray Distances \n Outlier Evaluation \nlog transformed data") +
  geom_text(aes(label = patient_id), size = 4, check_overlap = FALSE, vjust = -1) +
  facet_wrap(~Week) +
...
# No obvious sample outliers. Data just seems to be highly variable
...
Analysis of the numbers of taxa detected (Richness) and their diversity (Shannon Diversity). Other measures (Simpsons, Chao1, ACE, etc.) can also be calculated by changing the measures argument. See ?plot_richness for details.

A table of alpha diversity measures is also created which can then be imported into Excel or Prism for further analysis.

```{r Alpha Diversity Analysis, fig.width=8, fig.height=6}
...
Analysis of per sample community composition relationships. This is an exploratory process. One should consider what each distance measure means in the context of their data set. It is good practice to:

1. Compare several distance measures
2. Explore several PC axis

```{r Beta Diversity Analysis, fig.width=8, fig.height=8}
#Ordination Analysis
...
```{r Publication PcoA, fig.width=8, fig.height=8}
#  Single PCoA lot for presentation / publication
plot_ordination(ps0.log, ord.pcoa.wuni, color = "VL_category") +
  theme_bw() +
  geom_point(size=4, alpha = 0.7) +
  scale_color_brewer(palette = "Dark2") +
  geom_point(colour = "grey50", size = 1.5) +
...
#################################################
#                                               #
# PERMANOVA | ADONIS Group Significance Testing #
#                                               #
#################################################

Signficance testing of groups. The beginning of this code is a function called *doadonis* that can be run on any data set. You can supply a variable to compare if there is a significant difference between two groups.
...
Constrained ordination using supplied variables. Variable should ideally be selected based on knowledge of the system at hand. In this case, we know CD4 T cell abundance and HIV viral load contribute to the disease pathology observed in the clinic, so we constrain the ordination along these two variables.

```{r CCA, fig.width=8, fig.height=6}
...
```{r DiffAb Baseline Samples, fig.width=8, fig.height=6}
...
```{r DiffAb Shift, fig.width=8, fig.height=6}
...
```{r Random Forest, fig.width=8, fig.height=6}
# Random Forest
...
```{r}
# Function to create rlog normalized data
...
```{r DeSeq2 Identified Taxa by Viral Load Category, fig.width=9, fig.height=6}
# Plots of DeSeq2 identified taxa

# Transformations of data with samples removed with no recorded VL
ps0.base.no_vl_nc.ra <- transform_sample_counts(ps0.base.no_vl_nc, function(x) {x/sum(x)} )
ps0.base.no_vl_nc.rlog <- rlog_norm_otus(ps0.base.no_vl_nc, remove_negs = TRUE)

ps0.no_vl_nc.ra <- transform_sample_counts(ps0.no_vl_nc, function(x) {x/sum(x)} )
ps0.no_vl_nc.rlog <- rlog_norm_otus(ps0.no_vl_nc, remove_negs = TRUE)
