...
ps0 <- readRDS("~/Dropbox/Research/AIDS/HIV/presti/HIV_oral/dada2/data/ps0.HIV_oral.gg.RDS")
ps0
...
p.ss.violin <- ggplot(ss.df, aes_string(x, y = "RSVs", fill = x)) + # x is what you assigned it above
  ...
  geom_text(data = subset(ss.df, RSVs <= y), aes_string(x, y="RSVs", label=label), size=2) # This labels a subset that fall below threshold variable y and labels them with the label variable
p.ss.violin
...
p.cd4 <- ggboxplot(ps1.rich, x = "Week", y = "cd4", add = "jitter", color = "Week") +
  stat_compare_means(label = "p.format") +
  ylab("CD4 (c/ml)")

p.vl <- ggboxplot(ps1.rich, x = "Week", y = "vl", add = "jitter", color = "Week") +
  stat_compare_means(label = "p.format") +
  scale_y_log10() +
...
```{r alpha-diversity}
# CD4 category
p.rich.CD4 <- ggboxplot(ps1.rich, x = "cd4_category", y = "richness_0", notch = FALSE, add = "jitter") +
  facet_grid(~Week) +
  ylab("Richness") +
  theme(axis.title.x = element_blank())
  stat_compare_means(method = "t.test", label = "p.format")

p.shannon.CD4 <- ggboxplot(ps1.rich, x = "cd4_category", y = "diversities_shannon", notch = FALSE, add = "jitter") +
  facet_grid(~Week) +
...
p.rich.VL <- ggboxplot(ps1.rich, x = "vl_category", y = "richness_0", notch = FALSE, add = "jitter") +
  facet_grid(~Week) +
  ylab("Richness") +
  theme(axis.title.x = element_blank())
  stat_compare_means(method = "t.test", label = "p.format")

p.shannon.VL <- ggboxplot(ps1.rich, x = "vl_category", y = "diversities_shannon", notch = FALSE, add = "jitter") +
  facet_grid(~Week) +
  ylab("Shannon Diversity") +
  theme(axis.title.x = element_blank()) 
  stat_compare_means(method = "t.test", label = "p.format")
...
p.paired.rich.vl <- ggpaired(ps1.rich, x = "Week", y = "richness_0", id = "patient_ID", facet.by = "vl_category") +
  stat_compare_means(paired = TRUE) +
  ylab("Richness")

p.paired.rich.cd4 <- ggpaired(ps1.rich, x = "Week", y = "richness_0", id = "patient_ID", facet.by = "cd4_category") +
  stat_compare_means(paired = TRUE) +
  ylab("Richness")
...
# Richness
p.regr.rich.base.cd4 <- plot_regression(richness_0 ~ cd4, meta(ps1.rich.base), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
  ggtitle("Baseline") +
  ylab("Richness") +
  xlab("CD4 (c/ml)") +
  ylim(0,200) +
...
p.regr.rich.24.cd4 <- plot_regression(richness_0 ~ cd4, meta(ps1.rich.24), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
  ggtitle("Week 24") +
  ylab("Richness") +
  xlab("CD4 (c/ml)") +
  ylim(0,200) +
...
# Shannon Diversity
p.regr.sd.base.cd4 <- plot_regression(diversities_shannon ~ cd4, meta(ps1.rich.base), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
  ggtitle("Baseline") +
  ylab("Shannon Diversity") +
  xlab("CD4 (c/ml)") +
  ylim(0,5) +
...
p.regr.sd.24.cd4 <- plot_regression(diversities_shannon ~ cd4, meta(ps1.rich.24), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
  ggtitle("Week 24") +
  ylab("Shannon Diversity") +
  xlab("CD4 (c/ml)") +
  ylim(0,5) +
...
grid.arrange(p.regr.sd.base.cd4, p.regr.sd.24.cd4, ncol = 2)

```

```{r alpha-diversity-regression-VL}
# VL
p.regr.rich.base.vl <- plot_regression(richness_0 ~ vl, meta(ps1.rich.base), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
...
```{r relative-importance-analysis, eval=FALSE, include=FALSE}
# Baseline
fit.base <- lm(cd4 ~ diversities_shannon + richness_0 + tnf2 + tnf1 + vl + sCD14, data=ps1.rich.base)
fit.24 <- lm(cd4 ~ diversities_shannon + richness_0 + tnf2 + tnf1 + sCD14, data=ps1.rich.24)

# Calculate relative importance of each predictor
calc.relimp(fit.base, type = c("lmg", "last", "first", "pratt"), rank = TRUE)

# Bootstrap Measures of Relative Importance (1000 samples) 
boot.base <- boot.relimp(fit.base, b = 1000, type = c("lmg", 
  "last", "first", "pratt"), rank = TRUE, 
  diff = TRUE, rela = TRUE)
booteval.relimp(boot.base) # print result
plot(booteval.relimp(boot.base,sort=TRUE)) # plot result

# Week 24
# Calculate relative importance of each predictor
calc.relimp(fit.24, type = c("lmg", "last", "first", "pratt"), rank = TRUE)

# Bootstrap Measures of Relative Importance (1000 samples) 
boot.24 <- boot.relimp(fit.24, b = 1000, type = c("lmg", 
  "last", "first", "pratt"), rank = TRUE, 
  diff = TRUE, rela = TRUE)
booteval.relimp(boot.24) # print result
plot(booteval.relimp(boot.24,sort=TRUE)) # plot result

```

```{r influential-observations, eval=FALSE, include=FALSE}
avPlots(fit.base)
avPlots(fit.24)

# Influence Plot 
influencePlot(fit.base,	id.method="identify", main="Influence Plot", sub="Circle size is proportial to Cook's Distance" )
influencePlot(fit.24,	id.method="identify", main="Influence Plot", sub="Circle size is proportial to Cook's Distance" )
...
```{r proscutes, eval=FALSE, include=FALSE}
# Remove Week 24 Sample 16785
ps1.24.log
ps1.24.log.1 <- subset_samples(ps1.24.log, Description != "83772_Week_24_1")
ps1.24.log.1

ord.base <- ordinate(ps1.base.log, method = "PCoA", distance = "wunifrac")
ord.24 <- ordinate(ps1.24.log.1, method = "PCoA", distance = "wunifrac")

# Join sample data and ordination axes together in one data.table
ord.base.dt <- data.table(ord.base$vectors, keep.rownames = TRUE)
setkey(ord.base.dt, "rn", "SampleID")
ord.24.dt <- data.table(ord.24$vectors, keep.rownames = TRUE)

ordWUdt <- data.table(ord.base$vectors, keep.rownames = TRUE)
setnames(ordWUdt, "rn", "SampleID")
setkey(ordWUdt, SampleID)


pro1 <- ade4::procuste(dfX = ord.base.dt[, list(Axis.1, Axis.2)],
                       dfY = ord.24.dt[, list(Axis.1, Axis.2)])


# Add the procruste rotated and scaled coordinates to each table
ord.base.dt[, c("New1", "New2") := pro1$tabX]
ord.24.dt[, c("New1", "New2") := pro1$tabY]

# Combine the tables for plottiing
ord.base.dt[, D := "Baseline"]
ord.24.dt[, D := "Week 24"]
keepCols = c("New1", "New2", "D")
procrustdt = rbindlist(list(ord.base.dt[, keepCols, with = FALSE],
                            ord.24.dt[, keepCols, with = FALSE]))
...
x.ds.week = tapply(sigtab_dds.week$log2FoldChange, sigtab_dds.week$Family, function(x) max(x))
x.ds.week = sort(x.ds.week, TRUE)
sigtab_dds.week$Species = factor(as.character(sigtab_dds.week$Family), levels=names(x.ds.week))
...
dds.24 <- DESeq(ds.24, test="Wald", fitType="parametric", betaPrior = FALSE) # Worth trying parametric and local fitTypes
...
```{r indicator-species-analysis-week}
# Working with native vegan code, so convert phyloseq object to data.frame
otu.table <- data.frame(otu_table(ps1))

# make a data frame from the sample_data
sampledf <- data.frame(sample_data(ps1))

# Indicator species analysis using multipatt
indicator.species <- multipatt(otu.table, sampledf$Week, func="IndVal.g", duleg=TRUE)

# multipatt() reports the square-root of indval, so the original version of the indicator species value would be:
indval <- indicator.species$str^2

# IndVal is calculated as the product of fidelity ("B"; occupancy) and specificity ("A"; mean abundance in a group, normalized to the sum of
# all group mean abundances - can be thought of as "I found one of these - what is the probability that I am in a particular group
# or habitat type"):
B <- indicator.species$B
A <- indicator.species$A

# Significance table
sig <- indicator.species$sign

# Ouput accounting forFDR
indicator.results<-data.frame(indval, B, A, sig)
indicator.results$Pfdr<-p.adjust(indicator.results$p.value, "fdr")

significant.indicators<-indicator.results[indicator.results$Pfdr<0.05, ]
significant.indicators

# point biserial correlation between presence/absence of a species and a habitat type

pa <- decostand(otu.table, method = "pa")
phi_coeff <- multipatt(pa, sampledf$SurvivalStatus, func="r.g", duleg=TRUE)
...
```{r indicator-species-analysis-cd4-week-24}
# Working with native vegan code, so convert phyloseq object to data.frame
otu.table.24 <- data.frame(otu_table(ps1.24))

# make a data frame from the sample_data
sampledf.24 <- data.frame(sample_data(ps1.24))

# Indicator species analysis using multipatt
indicator.species.24 <- multipatt(otu.table.24, sampledf.24$cd4_category, func="IndVal.g", duleg=TRUE)

# multipatt() reports the square-root of indval, so the original version of the indicator species value would be:
indval.24 <- indicator.species.24$str^2

# IndVal is calculated as the product of fidelity ("B"; occupancy) and specificity ("A"; mean abundance in a group, normalized to the sum of
# all group mean abundances - can be thought of as "I found one of these - what is the probability that I am in a particular group
# or habitat type"):
B.24 <- indicator.species.24$B
A.24 <- indicator.species.24$A

# Significance table
sig.24 <- indicator.species.24$sign

# Ouput accounting forFDR
indicator.results.24 <- data.frame(indval.24, B.24, A.24, sig.24)
indicator.results.24$Pfdr<-p.adjust(indicator.results.24$p.value, "fdr")

significant.indicators.24<-indicator.results.24[indicator.results.24$Pfdr<0.05, ]
significant.indicators.24

# point biserial correlation between presence/absence of a species and a habitat type
pa.24 <- decostand(otu.table.24, method = "pa")
phi_coeff.24 <- multipatt(pa.24, sampledf.24$cd4_category, func="r.g", duleg=TRUE)

summary(phi_coeff.24)
...
# Genus = Treponema
ps1_Treponema <- ps1.ra %>%
    subset_taxa(Genus == "Treponema") %>%
    psmelt() 

# Genus = Muribaculum
ps1_Muribaculum <- ps1.ra %>%
  subset_taxa(Family == "Porphyromonadaceae") %>%
  psmelt()

# Treponema violin-point plots
p.Treponema <- ggplot(ps1_Treponema, aes(x = cd4_category, y = Abundance, fill = Genus)) + 
...
# Muribaculum violin-point plots
p.Muribaculum <- ggplot(ps1_Muribaculum, aes(x = cd4_category, y = Abundance, fill = Genus)) + 
  scale_size_continuous(range = c(2, 9)) +
  geom_violin(na.rm = TRUE, alpha = 0.5, fill = "grey", draw_quantiles = TRUE) +
  facet_grid(Week~Species, scales = "free_x", space = "free_x") +
  scale_y_log10() +
