...
ps0 <- readRDS("./data/ps0.HIV_oral.gg.RDS")
ps0

# Load mapping file
map <- import_qiime_sample_data("./data/HIV_oral_mapping.txt")
...
p.regr.rich.base.cd4 <- plot_regression(Richness ~ cd4, meta(ps1.rich.base), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
  ...
  ylab("Richness") +
  xlab("CD4 (c/ml)") +
  annotate("text", x = 775, y = 70, label = "p = 0.8219") +
  ylim(0,75) +
  xlim(0,900)
...
p.regr.rich.24.cd4 <- plot_regression(Richness ~ cd4, meta(ps1.rich.24), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
  ...
  ylab("Richness") +
  xlab("CD4 (c/ml)") +
  annotate("text", x = 775, y = 70, label = "p = 0.0658") +
  ylim(0,75) +
  xlim(0,900)
...
set.seed(2)
# Baseline samples
output.forest.base <- randomForest(CD4_category ~  Shannon + Richness + VL + tnf1 + tnf2 + scd14, na.action = na.omit, data = ps0.rich.base)
...
# Week 24 samples
output.forest.24 <- randomForest(CD4_category ~ Shannon + Richness + VL + tnf1 + tnf2 + scd14, na.action = na.omit, data = ps0.rich.24)
...
# Baseline
fit.base <- lm(CD4 ~ Shannon + Richness + tnf2 + tnf1 + scd14 + VL, data=ps0.rich.base)
fit.24 <- lm(CD4 ~ Shannon + Richness + tnf2 + tnf1 + scd14 + VL, data=ps0.rich.24)
...
library(caret)
myfolds <- createMultiFolds(ps0.rich.base$CD4_category, k = 5, times = 10)
control <- trainControl("repeatedcv", index = myfolds, selectionFunction = "oneSE")

mod1 <- train(CD4_category ~ Shannon + Richness + tnf1 + tnf2 + scd14 + IL6 +VL, data = ps0.rich.base, method = "pls", preProc = c("center", "scale"), na.action = na.omit, tuneLength = 25)
plot(mod1)

mod2 <- train(CD4_category ~ Shannon + Richness, data = ps0.rich.base, method = "lda", metric = "accuracy")
...
p.ord.CD4 <- plot_ordination(ps0.rem_nc, ord.ps0.wuni, color = "Week") +
  geom_point(size=3, alpha = 0.7) +
  scale_color_brewer(palette = "Dark2") +
  geom_point(colour = "grey50", size = 1.5) +
  facet_grid(~ CD4_category)

p.ord.VL <- plot_ordination(ps0.rem_nc, ord.ps0.wuni, color = "Week") +
  geom_point(size=3, alpha = 0.7) +
  scale_color_brewer(palette = "Dark2") +
  geom_point(colour = "grey50", size = 1.5) +
...
p.connect <- plot_ordination(ps0.rem_nc, ord.ps0.wuni, color = "Week") +
  geom_point(size=3, alpha = 0.7) +
  scale_color_brewer(palette = "Dark2") +
  geom_point(colour = "grey50", size = 1.5) +
  geom_line(aes(group = patient_id), color = "black")
...
ps0_Capnocytophaga <-ps0.base.no.vl.nc.ra %>%
    subset_taxa(Genus == "Capnocytophaga") %>%
    psmelt() 

# Genus = Aggregatibacter
ps0_Aggregatibacter <- ps0.base.no.vl.nc.ra %>%
  subset_taxa(Genus == "Aggregatibacter") %>%
  psmelt()

# Family = [Paraprevotellaceae]
ps0_Paraprevotellaceae <- ps0.base.no.vl.nc.ra %>%
  subset_taxa(Family == "[Paraprevotellaceae]") %>%
  psmelt()

# Genus = Dialister
ps0_Dialister <- ps0.base.no.vl.nc.ra %>%
  subset_taxa(Genus == "Dialister") %>%
  psmelt()

# Genus = Fusobacterium
ps0_Fusobacterium <- ps0.base.no.vl.nc.ra %>%
  subset_taxa(Genus == "Fusobacterium") %>%
  psmelt()

# Genus = Veillonella
ps0_Veillonella <- ps0.base.no.vl.nc.ra %>%
  subset_taxa(Genus == "Veillonella") %>%
  psmelt()

# Genus = Streptococcus
ps0_Streptococcus <- ps0.base.no.vl.nc.ra %>%
  subset_taxa(Genus == "Streptococcus") %>%
  psmelt()

# Genus = Neisseria
ps0_Neisseria <- ps0.base.no.vl.nc.ra %>%
  subset_taxa(Genus == "Neisseria") %>%
  psmelt()

# Genus = Prevotella
ps0_Prevotella <- ps0.base.no.vl.nc.ra %>%
