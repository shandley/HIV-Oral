...
sd.div <- merge(sd.1, diversity, by ="row.names") # merge ss with sd by row names. Rename ss to RSVs in the new data frame
...
est.rich <- estimate_richness(ps1, measures = c("Observed", "Shannon"))
head(est.rich)

# Make new data table combining richness with sample data
ps1.rich <- cbind(sample_data(ps1), diversity)
head(ps1.rich)
...
p.regr.rich.base.cd4 <- plot_regression(richness_0 ~ cd4, meta(ps1.rich.base), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
  ...
  ylab("Richness") +
  xlab("CD4 (c/ml)") +
  annotate("text", x = 775, y = 70, label = "p = 0.9207") +
  ylim(0,75) +
  xlim(0,900)
...
p.regr.rich.24.cd4 <- plot_regression(richness_0 ~ cd4, meta(ps1.rich.24), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
  ...
  ylab("Richness") +
  xlab("CD4 (c/ml)") +
  annotate("text", x = 775, y = 70, label = "p = 0.1558") +
  ylim(0,75) +
  xlim(0,900)
...
```{r relative-importance-analysis}
# Baseline
...
```{r indicator-species-analysis-cd4-base}
# Working with native vegan code, so convert phyloseq object to data.frame
otu.table.base <- data.frame(otu_table(ps1.base))

# make a data frame from the sample_data
sampledf.base <- data.frame(sample_data(ps1.base))

# Indicator species analysis using multipatt
indicator.species.base <- multipatt(otu.table.base, sampledf.base$cd4_category, func="IndVal.g", duleg=TRUE)

# multipatt() reports the square-root of indval, so the original version of the indicator species value would be:
indval.base <- indicator.species.base$str^2

# IndVal is calculated as the product of fidelity ("B"; occupancy) and specificity ("A"; mean abundance in a group, normalized to the sum of
# all group mean abundances - can be thought of as "I found one of these - what is the probability that I am in a particular group
# or habitat type"):
B.base <- indicator.species.base$B
A.base <- indicator.species.base$A

# Significance table
sig.base <- indicator.species.base$sign

# Ouput accounting forFDR
indicator.results.base<-data.frame(indval.base, B.base, A.base, sig.base)
indicator.results.base$Pfdr<-p.adjust(indicator.results.base$p.value, "fdr")

significant.indicators.base<-indicator.results.base[indicator.results.base$Pfdr<0.05, ]
significant.indicators.base
