...
# ps1.CD4.shift <- subset_samples(ps1, CD4_shift_comparison != "exclude")
# ps1.CD4.shift
# ps1.CD4.shift <- prune_samples(sample_sums(ps1.CD4.shift) > 0, ps1.CD4.shift)
# ps1.CD4.shift.log <- transform_sample_counts(ps1.CD4.shift, function(x) log(1 + x))
...
ggplot(ps1_phylum, aes(x = sample_no, y = Abundance, fill = Phylum)) + 
  theme_bw() +
  geom_bar(stat = "identity") +
...
ps1.rich <- cbind(sample_data(ps1), est.rich)
head(ps1.rich)

# log10 transform skewed viral load data
ps1.rich$vl <- log10(ps1.rich$vl)

p.cd4 <- ggpaired(ps1.rich, x = "Week", y = "cd4", add = "jitter", color = "Week", line.color = "gray", line.size = 0.4) +
  stat_compare_means(method = "t.test", label = "p.format", paired = TRUE) +
  ylab("CD4 (c/ml)")

p.vl <- ggpaired(ps1.rich, x = "Week", y = "vl", add = "jitter", color = "Week", line.color = "gray", line.size = 0.4) +
  stat_compare_means(method = "t.test", label = "p.format", paired = TRUE) +
...
```{r alpha-diversity}
# CD4 category
p.rich.CD4 <- ggboxplot(ps1.rich, x = "cd4_category", y = "Observed", notch = FALSE, add = "jitter") +
  facet_grid(~Week) +
  stat_compare_means(method = "t.test", label = "p.format")

p.shannon.CD4 <- ggboxplot(ps1.rich, x = "cd4_category", y = "Shannon", notch = FALSE, add = "jitter") +
  facet_grid(~Week) +
  stat_compare_means(method = "t.test", label = "p.format")

grid.arrange(p.rich.CD4, p.shannon.CD4, ncol = 2)

p.rich.VL <- ggboxplot(ps1.rich, x = "vl_category", y = "Observed", notch = FALSE, add = "jitter") +
  facet_grid(~Week) +
  stat_compare_means(method = "t.test", label = "p.format")

p.shannon.VL <- ggboxplot(ps1.rich, x = "vl_category", y = "Shannon", notch = FALSE, add = "jitter") +
  facet_grid(~Week) +
  stat_compare_means(method = "t.test", label = "p.format")
...
# CD4
p.regr.rich.base.cd4 <- plot_regression(Richness ~ cd4, meta(ps1.rich.base), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
  ...
  ylim(0,75) +
  xlim(0,900)
cor.test(sample_data(ps1.rich.base)$Richness, sample_data(ps1.rich.base)$cd4)

p.regr.rich.24.cd4 <- plot_regression(Richness ~ cd4, meta(ps1.rich.24), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
  ...
  ylim(0,75) +
  xlim(0,900)
cor.test(sample_data(ps1.rich.24)$Richness, sample_data(ps1.rich.24)$cd4)
...
# VL
p.regr.rich.base.vl <- plot_regression(Richness ~ vl, meta(ps1.rich.base), B=500, show.lm = TRUE, spag = FALSE, shade = FALSE) +
  ggtitle("Richness vs. VL (c/ml) Baseline") +
  ylab("Richness") +
  xlab("VL (c/ml)") +
  annotate("text", x = 5.5, y = 175, label = "p = 0.9209") +
  ylim(0,200)
cor.test(sample_data(ps1.rich.base)$Richness, sample_data(ps1.rich.base)$vl)
...
ancova(Shannon~cd4*Week, data = ps1.rich)
...
ancova(Richness~cd4*Week, data = ps1.rich)
...
# Interaction between age, EE status and alpha diversity
# Or use hotdog to visualize the resutls
ancova(Shannon~vl*Week, data = ps1.rich)
...
ancova(Richness~vl*Week, data = ps1.rich)
...
set.seed(2)
# Baseline samples
output.forest.base <- randomForest(cd4_category ~  Shannon + Richness + vl + tnf1 + tnf2, na.action = na.omit, data = ps1.rich.base)
...
# Week 24 samples
output.forest.24 <- randomForest(cd4_category ~  Shannon + Richness + vl + tnf1 + tnf2, na.action = na.omit, data = ps1.rich.24)
...
p.rf.24 <- ggplot(importance.molten.24.sort, aes(x = Var1, y = value)) +
  ...
  ylab("Mean Decrease Gini (Importance") +
  theme(axis.title.x = element_blank()) +
  annotate("text", x = 5, y = 2.5, label = "OOB error = 13.79%")
...
# Baseline
fit.base <- lm(cd4 ~ Shannon + Richness + tnf2 + tnf1 + vl, data=ps1.rich.base)
fit.24 <- lm(cd4 ~ Shannon + Richness + tnf2 + tnf1, data=ps1.rich.24)
...
p.ord.CD4 <- plot_ordination(ps1, ord.ps1.wuni, color = "cd4_category") +
  geom_point(size=3, alpha = 0.7) +
  scale_color_brewer(palette = "Dark2") +
  geom_point(colour = "grey50", size = 1.5) +
  facet_grid(~ Week) +
  stat_ellipse(geom = "polygon", alpha = 1/10, aes(fill = cd4_category))
...
p.connect <- plot_ordination(ps1, ord.ps1.wuni, color = "Week") +
  geom_point(size=3, alpha = 0.7) +
  scale_color_brewer(palette = "Dark2") +
  geom_point(colour = "grey50", size = 1.5) +
  geom_line(aes(group = patient_ID), color = "black")
